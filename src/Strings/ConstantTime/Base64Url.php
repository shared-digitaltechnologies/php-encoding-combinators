...
abstract class Base64Url
{
    ...
    {
        try {
            return sodium_bin2base64($value, SODIUM_BASE64_VARIANT_URLSAFE);
    ...
    {
        try {
            return sodium_base642bin($value, SODIUM_BASE64_VARIANT_URLSAFE, $ignore);
        } catch (SodiumException $exception) {
    ...
    {
        try {
            return sodium_base642bin($value, SODIUM_BASE64_VARIANT_URLSAFE_NO_PADDING, $ignore);
        } catch (SodiumException $exception) {
            throw new WrappedDecodeException($exception);
        }
    }
